...
import ums2.util.UserTableModel;

public class ShowUserTableDialog extends JDialog{	
	...
	public ShowUserTableDialog(JFrame parent, String msg, List list){
	...
	private void showTableData(int pageNo){
		...
		UserTableModel model = new UserTableModel(currentPageUsers);
		//3.为JTable设置TableModel：指定行、列信息
		table.setModel(model);		
		
		//4.设置JTable的列宽
		table.getColumn("ID").setPreferredWidth(30);
		table.getColumn("email").setPreferredWidth(130);
