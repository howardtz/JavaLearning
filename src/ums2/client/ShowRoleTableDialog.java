...
import ums2.util.RoleTableModel;

public class ShowRoleTableDialog extends JDialog{
	private static final long serialVersionUID = 1L;
	private int windowHeight = 180;  //窗口高度		
	private int windowWidth = 380;   //窗口宽度	
	private JTable table = new JTable();	
	private List list;	//角色列表
	...
	public ShowRoleTableDialog(JFrame parent, String msg, List list){
		super(parent, "角色列表",true);
		this.list = list;
	...
	private void showTableData(int pageNo){
		...
		RoleTableModel model = new RoleTableModel(currentPageUsers);
		//3.为JTable设置TableModel：指定行、列信息
		table.setModel(model);		
		
		//4.设置JTable的列宽
		table.getColumn("序号").setPreferredWidth(30);
		table.getColumn("角色名称").setPreferredWidth(80);
		table.getColumn("描述").setPreferredWidth(140);
		table.getColumn("权限").setPreferredWidth(120);
